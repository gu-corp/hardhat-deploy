                ...
                case 'string':
                    ...
                    if (protocolSplit.length > 1) {
                        if (protocolSplit[0].toLowerCase() === 'external') {
                            ...
                            addressesToProtocol[address.toLowerCase()] = `awskms://${addressSplit[1]}:${addressSplit[2]}:${addressSplit[3]}:${addressSplit[4]}`;
                        }
                        else {
...
function filterABI(abi, excludeSighashes) {
    return abi.filter((fragment) => fragment.type !== 'function' ||
        !excludeSighashes.has(abi_1.Interface.getSighash(abi_1.Fragment.from(fragment))));
}
